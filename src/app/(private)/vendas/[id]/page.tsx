  ...
  return (
    <section className="mt-6 mb-4">
      <div className="mt-6 mb-4 flex items-center justify-between">
        ...
        <Link href={`/vendas/${order.id}/editar`} className="text-xs cursor-pointer mr-2">
          editar
        </Link>
      </div>
      ...
      <div className="flex flex-col px-1 pt-1 items-center">
        <div className="relative mt-2 ml-auto mr-auto p-4 flex flex-col items-center justify-start w-140 h-fit border">
          <p className="absolute top-5 text-sm font-normal">{statusLabel[order.status]}</p>

          <p className="mt-10 sm:mt-12 text-4xl text-center">pedido #{order.order_number}</p>
          <p className="mt-2">{order.customer.name}</p>

          <div className="mt-5 w-fit h-fit border-b border-secondary/50">
            <p className="py-0.5 text-[12px]">{formatDateTime(order.order_date)}</p>
          </div>
          ...
          <div className="relative w-full sm:w-100 h-5 py-4 flex items-center justify-between">
            ...
            <p className="text-sm font-light">{formatBRL(order.discount_amount ?? "0")}</p>
          </div>
          <hr className="border-t border-tertiary/25 w-full sm:w-100" />

          <div className="relative w-full sm:w-100 h-5 py-4 flex items-center justify-between">
            <p className="text-sm font-light">total</p>
            <p className="text-sm font-light">{formatBRL(order.total_amount)}</p>
          </div>

          <div className="relative mt-6 w-full sm:w-100 px-2 py-3 flex flex-col gap-2 border-t border-tertiary/25">
            <p className="text-xs uppercase">itens</p>
            {order.items.map((item) => (
              <div key={item.id} className="flex items-center justify-between text-sm font-light">
                <p>
                  {item.quantity}x {item.product}
                </p>
                <p>{formatBRL(item.total)}</p>
              </div>
            ))}
          </div>

          <div className="relative mt-2 w-full sm:w-100 px-2 py-3 flex flex-col gap-2 border-t border-tertiary/25">
